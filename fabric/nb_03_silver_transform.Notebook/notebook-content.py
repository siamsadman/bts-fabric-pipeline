...
spark.table("load_watermark").orderBy("year", "month").show()

(spark.table("fact_flight")
   .groupBy("_source_file").count().orderBy("_source_file").show())
...
loaded = {(2020,1), (2020,2), (2020,3), (2020,4), (2020,5), (2023,6)}
...
print(json.dumps(periods, separators=(",", ":")))
...
w = spark.table("load_watermark")
print("watermark rows:", w.count())
print("period range  :", w.agg(F.min("year"), F.max("year")).collect()[0])

f = spark.table("fact_flight")
print()
print("fact_flight rows          :", f"{f.count():,}")
print("fact_delay_attribution    :", f"{spark.table('fact_delay_attribution').count():,}")
print("silver_flights            :", f"{spark.table('silver_flights').count():,}")
print("bronze_flights            :", f"{spark.table('bronze_flights').count():,}")
print()
print("distinct periods in fact  :", f.select('_source_file').distinct().count())
print("dim_airport               :", spark.table("dim_airport").count())
print("dim_carrier               :", spark.table("dim_carrier").count())
...
print("--- referential integrity, full dataset ---")
...
for key, dim, dim_key in [
    ...
    print(f"  {key:<20} -> {dim:<24} orphans: {orphans:>8}  {'OK' if orphans == 0 else 'FAIL'}")
